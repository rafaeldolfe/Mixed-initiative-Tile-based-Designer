...
public class LevelGenerator : MonoBehaviour
{
    ...
    {
        var tilemapParent = tilemap.transform.parent;
    ...
    public Level GenerateLevel(bool visualize = true, bool generateExtraWalls = true)
    {
        ...
        var state = sg.GenerateShape(visualize);
        ...
        foreach (var cell in state.space)
        {
    ...
    {
        for (int i = 0; i < num; i++) GenerateLevel(false);
    }
    ...
    private void PopulateWithWalls(List<Cell> space)
    {
        foreach (var cell in space)
        {
            ...
            {
                var roomShapeRoomsDict = roomTypeToDictionaryDict[cell.roomType];
            ...
            {
                var tilemapPosition = new Vector3Int(offsetX + i, offsetY + j, 0);
    ...
    private void LoadRooms()
    {
        ...
        var roomTemplates = Resources.LoadAll<RoomTemplateScriptableObject>(resourcePath);
        ...
        foreach (var room in roomTemplates)
        {
            ...
            {
                var roomShapeRoomsDict = roomTypeToDictionaryDict[room.type];
                if (roomTypeToDictionaryDict[room.type].ContainsKey(room.connections.GetRoomShape()))
                    if (roomShapeRoomsDict[room.connections.GetRoomShape()].Select(setting => setting.room)
                        .Contains(room))
                        continue;
            }
        ...
        foreach (var room in roomTemplates)
        {
            ...
            var shape = room.connections.GetRoomShape();
    ...
    private Room PlaceRandomRoomTemplate(Cell cell)
    {
        ...
        var roomsDictionary = roomTypeToDictionaryDict[cell.roomType];
        if (!roomsDictionary.ContainsKey(cell.connections.GetRoomShape()))
        {
            Debug.LogWarning(
                $"roomsDictionary did not contain shape {cell.connections.GetRoomShape()} for type {cell.roomType}");
        ...
        var settings = roomsDictionary[cell.connections.GetRoomShape()];
        var rooms = settings
        ...
        if (rooms.Count == 0)
            throw new Exception(
                $"No rooms exist for the given cell's room type {cell.roomType} and cell's shape {cell.connections.GetRoomShape()}");
        Profiler.EndSample();
        Profiler.BeginSample("rooms.PickRandom();");
        var room = rooms.PickRandom();
        Profiler.EndSample();
    ...
    private Room PlaceRoomTemplate(Cell cell, RoomTemplate roomTemplate)
    {
        ...
        var room = new Room {template = roomTemplate};

        var tilesDict = roomTemplate.tilesDict;
        for (int j = 0; j < roomTemplate.height; j++)
        for (int i = 0; i < roomTemplate.width; i++)
        {
            var tilemapPosition = new Vector3Int(offsetX + i, offsetY + j, 0);
            var localPosition = new Vector3Int(i, j, 0);
            bool success = tilesDict.TryGetValue(localPosition, out var tile);
            if (success)
            {
                ...
                var sTile = tile as ScriptableTile;
                var oTile = tile as OverflowTile;
                if (sTile != null)
                {
                    if (sTile.tileType == SimpleTileType.Entrance) level.entrance = tilemapPosition;
                    if (sTile.tileType == SimpleTileType.Goal) level.goal = tilemapPosition;
                    foreach (var special in cell.specialFunctionalities)
    ...
    private void AddVector3IntToGuid(Dictionary<string, List<Vector3Int>> keyValuePairs, Vector3Int position,
        string guid)
    {
        if (!keyValuePairs.ContainsKey(guid)) keyValuePairs[guid] = new List<Vector3Int>();
        keyValuePairs[guid].Add(position);
    }

    private void AlignRoomToCell(RoomTemplate room, Cell cell)
    {
        var transformDirection = GetTransformDirection(room, cell);
    ...
    private void RotateRoom(RoomTemplate room, Direction transformDirection)
    {
        ...
        var rotatedTiles = new Vector3IntToTileDictionary();
        foreach (var pair in room.tilesDict)
        {
            float halfWidth = ((float) room.width - 1) / 2;
            float halfHeight = ((float) room.height - 1) / 2;
            var rotatedPos = RotatePointAroundPivot(
            ...
            {
                var overflowTile = (OverflowTile) pair.Value;
    ...
    private Direction GetTransformDirection(RoomTemplate room, Cell cell)
    {
        ...
        for (int i = 0; i < directions.Count; i++)
        {
            ...
            var transformedDirections = room.connections.Select(dir => dir.TransformDirection(current)).ToList();

            if (cell.connections.TrueForAll(dir => transformedDirections.Contains(dir))) return current;
        }
